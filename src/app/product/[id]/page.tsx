    ...
    try {
      ...
      if (productData.vendorId) {
        ...
        const [vendorRes, recsRes, followRes] = await Promise.allSettled([
          ...
          token ? api.get('/vendor/followed') : Promise.reject('GUEST')
        ]);
...
<button 
  onClick={() => {
    // Priority: Slug > VendorId > fallback to 404/not-found logic
    const vendorIdentifier = vendorData?.slug || product.vendorId;
    if (vendorIdentifier) {
      router.push(`/vendors/${vendorIdentifier}`);
    } else {
      toast.error("Vendor profile temporarily unavailable");
    }
  }} 
  className="flex-1 px-6 py-2.5 border-2 border-zinc-900 rounded-full text-[10px] font-black uppercase"
>
  Shop All
</button>
               </div>
